...
export default function MensajesDeAndrea() {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`/api/andrea/messages/${id}`, { method: "DELETE" });
  ...
  const handleSaveEdit = async (id) => {
    try {
      await fetch(`/api/andrea/messages/${id}`, {
        method: "PUT",
            ...
            messages.map((msg) => (
              ...
              >
                {editingMessage === msg._id ? (
                  <div>
                    <textarea
                      className="w-full p-3 border-2 border-pink-300  text-black rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                      value={editedContent}
